...
namespace kfusion
{
  namespace device
  {
    ...
    texture<int, 1, cudaReadModeElementType> triTex; //textureÎªCUDA¶¨Òå
...
void
kfusion::device::generateTriangles (const PtrStep<short2>& volume, const DeviceArray2D<int>& occupied_voxels, const float3& volume_size, DeviceArray<PointType>& output)
{   
  int device;
